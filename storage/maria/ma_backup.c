...
int aria_get_capabilities(File kfile, const char *table_name, ARIA_TABLE_CAPABILITIES *cap)__attribute__((visibility("default"))) ;
int aria_get_capabilities(File kfile, const char *table_name, ARIA_TABLE_CAPABILITIES *cap)
{
  ...
  */
  ...
  if (cap->encrypted)
  {
    ...
    cap->crypt_page_header_space= share.crypt_page_header_space;
                    ...
                    uchar *buffer)
{
  ...
  for (;;)
  {
    ...
    if (length == cap->block_size)
    {
      ...
      if (cap->encrypted)
      {
        ...
        io_arg.pageno= block;
        share.crypt_data= cap->crypt_data;
        share.silence_encryption_errors= retry < MAX_RETRY ? 1 : 0;
        share.crypt_page_header_space= cap->crypt_page_header_space;

        error= ma_crypt_index_post_read_hook(0, &io_arg);
  ...
  my_printf_error(my_errno,
                  "Error reading index file %s  block %lld",
                  MYF(ME_FATAL|ME_ERROR_LOG),
                  share.open_file_name.str, block);
                   ...
                   uchar *buffer, size_t *bytes_read)
{
  ...
  *bytes_read= cap->block_size;
  for (;;)
  {
    ...
    if (length == cap->block_size)
    {
      ...
      if (cap->encrypted && (block % cap->bitmap_pages_covered))
      {
        ...
        error= ma_crypt_data_post_read_hook(0, &io_arg);
        my_afree(io_arg.crypt_buf);
        if (error == 0 ||
            (my_errno != HA_ERR_DECRYPTION_FAILED &&
             my_errno != HA_ERR_WRONG_CRC))
  ...
  my_printf_error(my_errno,
                  "Error reading data file %s  block %lld",
                  MYF(ME_FATAL|ME_ERROR_LOG),
                  share.open_file_name.str, block);
  DBUG_RETURN(HA_ERR_WRONG_CRC);
}
