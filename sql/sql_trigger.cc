                             ...
                             const LEX_CSTRING *db_cl_name)
{
  ...
  if (!client_cs_name ||
      resolve_charset(client_cs_name->str,
                      ...
                      &client_cs, MYF(utf8_flag)))
  {
    sql_print_warning("Trigger for table '%s'.'%s': "
                      ...
                      db_name, table_name,
                      client_cs_name ? client_cs_name->str : "");

    invalid_creation_ctx= TRUE;
  }

  if (!connection_cl_name ||
      resolve_collation(connection_cl_name->str,
                        ...
                        &connection_cl,MYF(utf8_flag)))
  {
    sql_print_warning("Trigger for table '%s'.'%s': "
                      ...
                      db_name, table_name,
                      connection_cl_name ? connection_cl_name->str : "");

    invalid_creation_ctx= TRUE;
  }

  if (!db_cl_name ||
      resolve_collation(db_cl_name->str, NULL, &db_cl, MYF(utf8_flag)))
  {
    sql_print_warning("Trigger for table '%s'.'%s': "
                      "invalid database_collation value (%s).",
                      db_name, table_name,
                      db_cl_name ? db_cl_name->str : "");
