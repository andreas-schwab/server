...
private:
  ...
  /**
    ...
    start iterting from the beginnning of the list
    for each item in m_fields making time complexity O(n^2).

    So, this itertor will get the next element or
    fetch ith element usgin elem(i) as needed instead
    of iterting over the list from the beginning everytime.
  */
...
public:
  Item_trigger_row(THD *thd, Name_resolution_context *context_arg,
                   ...
                   const LEX_CSTRING &field_name_arg,
                   privilege_t priv, const bool ro,
                   uint sp_var_idx= 0, const Sp_rcontext_handler *rh= nullptr,
                   const Type_handler *handler= nullptr,
                   uint pos_in_q= 0, uint len_in_q= 0)
  : Item_splocal(thd, rh, &field_name_arg, sp_var_idx,
                 handler, pos_in_q, len_in_q),
